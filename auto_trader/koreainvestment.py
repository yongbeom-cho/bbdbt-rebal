...
class KoreaInvestment:
    ...
    def issue_access_token(self):
        ...
        resp_data = resp.json()
        if 'error_code' in resp_data and resp_data['error_code'] == 'EGW00133':
            time.sleep(600)
            resp = requests.post(url, headers=headers, data=json.dumps(data))
            resp_data = resp.json()
    ...
    def fetch_price(self, symbol: str, exchange: str = "서울") -> dict:
        """국내주식시세/주식현재가 시세
           해외주식현재가/해외주식 현재체결가
        ...
        if exchange == "서울":
            ...
            return self.fetch_oversea_price(symbol, exchange)
    ...
    def fetch_oversea_price(self, symbol: str, exchange: str = "") -> dict:
        """해외주식현재가/해외주식 현재체결가
        Args:
            symbol (str): 종목코드
            exchange (str): 거래소
    ...
    def fetch_today_1m_ohlcv(self, symbol: str, to: str=""):
        ...
        while last_hour > "090100":
            ...
            # 1 minute ago
            dt2 = dt1 - delta
            last_to = dt2.strftime("%H%M%S")
            if last_to > to:
                break
            # request 1minute ohlcv
            output = self._fetch_today_1m_ohlcv(symbol, last_to)
            output2 = output['output2']
            last_hour = output2[-1]['stck_cntg_hour']
    ...
    def __fetch_today_1m_ohlcv(self, symbol: str, to: str):
        """국내주식시세/주식당일분봉조회
    ...
    def _fetch_today_1m_ohlcv(self, symbol: str, to: str):
        output = self.__fetch_today_1m_ohlcv(symbol, to)
        try_limit = 5
        try_cnt = 0
        while 'output2' not in output:
            try_cnt += 1
            sleep_time = try_cnt * 0.2
    ...
    def fetch_ohlcv(self, symbol: str, timeframe: str = 'D', start_day: str = "", end_day: str = "",
                    adj_price: bool = True, exchange: str = "서울") -> dict:
        ...
        if exchange == '서울':
            ...
            resp = self.fetch_ohlcv_overesea(symbol, timeframe, end_day, adj_price, exchange)
        return resp
    ...
    def fetch_symbols(self, exchange: str = "서울"):
    ...
    def fetch_balance(self, exchange: str = "서울") -> dict:
        """잔고 조회
        ...
        if exchange == '서울':
            ...
            data = self.fetch_balance_oversea(exchange=exchange)
            output['output1'] = data['output1']
            output['output2'] = data['output2']
            ...
            while data['tr_cont'] == 'M':
                ...
                data = self.fetch_balance_oversea(fk200, nk200, exchange=exchange)
                output['output1'].extend(data['output1'])
                output['output2'].extend(data['output2'])
    ...
    def fetch_present_balance(self, foreign_currency: bool = True, exchange: str = "미국전체") -> dict:
        """해외주식주문/해외주식 체결기준현재잔고
        Args:
            foreign_currency (bool): True: 외화, False: 원화
            exchange (str): 거래소 ("미국전체"이면 미국 전 거래소 포지션 일괄 조회)
        Returns:
            dict: _description_
        """
        ...
        if exchange in ["나스닥", "뉴욕", "아멕스", "미국전체"]:
            nation_code = "840"
            if exchange == "나스닥":
                market_code = "01"
            elif exchange == "뉴욕":
            ...
            nation_code = "344"
            market_code = "01"
        elif exchange in ["상해", "심천"]:
            nation_code = "156"
            market_code = "00"
        elif exchange == "도쿄":
            nation_code = "392"
            market_code = "01"
        elif exchange in ["하노이", "호치민"]:
            nation_code = "704"
            market_code = "01" if exchange == "하노이" else "02"
        else:
            nation_code = "000"
            market_code = "00"
    ...
    def fetch_balance_oversea(self, ctx_area_fk200: str = "", ctx_area_nk200: str = "", exchange: str = "") -> dict:
        """해외주식주문/해외주식 잔고
        Args:
        ...
        exchange_cd = EXCHANGE_CODE2[exchange]
        currency_cd = CURRENCY_CODE[exchange]
    ...
    def create_market_buy_order(self, symbol: str, quantity: int, exchange: str = "서울") -> dict:
        """시장가 매수
        ...
        if exchange == "서울":
            ...
            resp = self.create_oversea_order("buy", symbol, "0", quantity, "00", exchange)
        return resp

    def create_market_sell_order(self, symbol: str, quantity: int, exchange: str = "서울") -> dict:
        """시장가 매도
        ...
        if exchange == "서울":
            ...
            resp = self.create_oversea_order("sell", symbol, "0", quantity, "00", exchange)
        return resp

    def create_limit_buy_order(self, symbol: str, price: int, quantity: int, exchange: str = "서울") -> dict:
        """지정가 매수
        ...
        if exchange == "서울":
            ...
            resp = self.create_oversea_order("buy", symbol, price, quantity, "00", exchange)
        return resp

    def create_limit_sell_order(self, symbol: str, price: int, quantity: int, exchange: str = "서울") -> dict:
        """지정가 매도
        ...
        if exchange == "서울":
            ...
            resp = self.create_oversea_order("sell", symbol, price, quantity, "00", exchange)
        return resp
    ...
    def create_oversea_order(self, side: str, symbol: str, price: int,
                             quantity: int, order_type: str, exchange: str = "") -> dict:
        """해외주식주문/해외주식 주문
        ...
        exchange_cd = EXCHANGE_CODE3[exchange]
    ...
    def fetch_oversea_order_book(self, symbol: str, exchange: str = "") -> dict:
        """해외주식 호가 조회 (uapi/overseas-price/v1/quotations/inquire-asking-price)

        응답 output 필드:
          pask1~pask10 : 매도호가 1순위(최저)~10순위(최고)
          pbid1~pbid10 : 매수호가 1순위(최고)~10순위(최저)
        """
    ...
    def fetch_oversea_open_orders(
        self, symbol: str = "", ctx_area_fk200: str = "", ctx_area_nk200: str = "", exchange: str = ""
    ) -> dict:
        """해외주식 미체결 주문 조회 (uapi/overseas-stock/v1/trading/inquire-nccs)
        ...
        url = f"{self.base_url}/{path}"
        tr_id = "VTTS3018R" if self.mock else "TTTS3018R"
        exchange_cd = EXCHANGE_CODE3[exchange]
        headers = {
            "content-type": "application/json",
            "authorization": self.access_token,
    ...
    def cancel_oversea_order(self, order_no: str, symbol: str, quantity: int, exchange: str = "") -> dict:
        """해외주식 주문 취소 (uapi/overseas-stock/v1/trading/order-rvsecncl)"""
        return self._oversea_rvsecncl(
            order_no=order_no, symbol=symbol, quantity=quantity,
            price=0.0, dvsn="02", exchange=exchange,
        )

    def modify_oversea_order(
        self, order_no: str, symbol: str, quantity: int, price: float, exchange: str = ""
    ) -> dict:
        """해외주식 주문 정정 (uapi/overseas-stock/v1/trading/order-rvsecncl)"""
        return self._oversea_rvsecncl(
            order_no=order_no, symbol=symbol, quantity=quantity,
            price=price, dvsn="01", exchange=exchange,
        )

    def _oversea_rvsecncl(
        self, *, order_no: str, symbol: str, quantity: int, price: float, dvsn: str, exchange: str = ""
    ) -> dict:
        """해외주식 정정/취소 공통 구현.

        dvsn: "01"=정정, "02"=취소
        TR ID: TTTT1004U (실전) / VTTT1004U (모의) — 공식 docs 기준
        """
        path = "uapi/overseas-stock/v1/trading/order-rvsecncl"
        url = f"{self.base_url}/{path}"
        tr_id = "VTTT1004U" if self.mock else "TTTT1004U"
        exchange_cd = EXCHANGE_CODE3[exchange]
        data = {
            "CANO": self.acc_no_prefix,
            "ACNT_PRDT_CD": self.acc_no_postfix,
            ...
            "ORD_QTY": str(quantity),
            "OVRS_ORD_UNPR": f"{price:.2f}" if dvsn == "01" else "0",
            "MGCO_APTM_ODNO": "",
            "ORD_SVR_DVSN_CD": "0",
        }
        hashkey = self.issue_hashkey(data)
    ...
    def fetch_ohlcv_overesea(self, symbol: str, timeframe: str = 'D',
                             end_day: str = "", adj_price: bool = True, exchange: str = ""):
        """해외주식현재가/해외주식 기간별시세
        ...
        Args:
            ...
            end_day (str, optional): 조회종료일자 (YYYYMMDD)
            adj_price (bool, optional): False: 수정주가 미반영, True: 수정주가 반영
            exchange (str): 거래소
        """
        ...
        exchange_code = EXCHANGE_CODE4[exchange]
    ...
    def fetch_volume_rank(self, market: str="KOREA", rank: str="거래량", safe=True, vol_cnt: int=500000):
        ...
        exclude_code = "0000000000"
        if safe:
            exclude_code = "1111111111"
        ...
        params = {
            ...
            "FID_TRGT_EXLS_CLS_CODE": exclude_code,
            "FID_INPUT_PRICE_1": "",
            "FID_INPUT_PRICE_2": "",
            "FID_VOL_CNT": str(vol_cnt),
            "FID_INPUT_DATE_1": ""
        }
        resp = requests.get(url, headers=headers, params=params)
        return resp.json()
